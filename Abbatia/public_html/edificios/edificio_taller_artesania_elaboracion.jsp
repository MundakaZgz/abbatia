...
<div id="container">
    ...
    <div class="break"></div>
    <display:table name="requestScope.Edificio.contenido" uid="edificio_artesania_elaboracion" pagesize="10"
                   export="true" requestURI="/mostrarEdificio.do"
                   sort="list" class="contenidoedificio alignCenter">
        <display:setProperty name="export.xsl.filename" value="edificio_artesania_elaboracion.xsl"/>
        <display:setProperty name="export.pdf.filename" value="edificio_artesania_elaboracion.pdf"/>
        <display:setProperty name="export.csv.filename" value="edificio_artesania_elaboracion.csv"/>
        ...
        <display:column titleKey="edificios.abadia.tabla.opciones">
            <c:if test="${edificio_artesania_elaboracion.estado == 1}">
                <html:link action="/elaboracion_pausar" paramName="edificio_artesania_elaboracion"
                           paramProperty="idElaboracion" paramId="clave">
                    <html:img border="0" page="/images/iconos/16/pausar.gif"
                              altKey="tooltip.pausar"/>
                </html:link>
            </c:if>
            <c:if test="${edificio_artesania_elaboracion.estado == 7}">
                <html:link action="/elaboracion_reanudar" paramName="edificio_artesania_elaboracion"
                           paramProperty="idElaboracion" paramId="clave">
                    <html:img border="0" page="/images/iconos/16/reanudar.gif"
                              altKey="tooltip.reanudar"/>
                </html:link>
            </c:if>
            <html:link action="/elaboracion_eliminar" paramName="edificio_artesania_elaboracion"
                       paramProperty="idElaboracion" paramId="clave">
